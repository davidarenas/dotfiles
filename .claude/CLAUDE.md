...
// ✅ CORRECT - Import schemas from the shared schema/type package
...
## Development Workflow Philoshophy

### Refactoring

Evaluating refactoring opportunities is not optional. After achieving a green state/test passing and committing your work, you MUST assess whether the code can be improved. However, only refactor if there's clear value - if the code is already clean and expresses intent well, move on to the next test.
...
The key is to write clean, testable, functional code that evolves through small, safe increments. The implementation should be the simplest thing that fulfills all requirements. When in doubt, favor simplicity and readability over cleverness.
